 ...
 *
 * Directly reused from Translation Loop 0.5.1. Licensed under MIT;
...
(function installPromptStackerRunner(globalScope) {
  ...
  function createRunController() {
    ...
    let generation = 0;
    return {
      start() { generation += 1; cancel = false; runState = "running"; },
      pause() { if (runState === "running") runState = "paused"; },
      resume() { if (runState === "paused") runState = "running"; },
      stop() { generation += 1; cancel = true; runState = "idle"; },
      get state() { return runState; },
      get cancelled() { return cancel; },
  ...
  function createRunner(options = {}) {
    ...
    function firstMatch(selectors, root = documentRef) {
      ...
      for (const selector of selectors) {
        ...
        for (const node of nodes) if (outsideOwnUi(node)) return node;
      }
      return null;
    }

    function getEditor() { return firstMatch(adapter.editor); }
    ...
    function setPromptText(text, editor = getEditor()) {
      ...
      let inserted = false;
      try { inserted = documentRef.execCommand("insertText", false, text); } catch (_) { inserted = false; }
    ...
    async function submit(prompt, submitOptions = {}) {
      ...
      if (!controller.canRun(runGeneration)) return { ok: false, error: "runnerが停止中" };
      ...
      if (!editor) return { ok: false, error: controller.cancelled ? "送信処理が停止された" : "入力欄が見つからない" };
      ...
      if (!readyButton && !allowEnterFallback) return { ok: false, error: "送信ボタンが有効にならない", activation: null, button: null };
      if (!controller.canRun(runGeneration)) return { ok: false, error: "送信処理が停止された" };
      const activation = clickSend(editor, { allowEnterFallback, button: readyButton });
    ...
    return {
      start: () => controller.start(), pause: () => controller.pause(), resume: () => controller.resume(), stop: () => controller.stop(),
      get state() { return controller.state; }, get cancelled() { return controller.cancelled; },
      getEditor, getSendButton, editorText, setPromptText, clickSend, submit
    };
  }

  const api = { DEFAULT_ADAPTER, normalizeText, createRunController, waitFor, createRunner };
