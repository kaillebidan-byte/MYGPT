...
assert.equal(adapter.PROMPT_ROOT_SELECTOR, '#prompt-textarea');
...
assert.match(source, /function getPromptRoot/);
assert.match(source, /function composerStructuredText/);
assert.match(source, /paragraphs\.map/);
assert.match(source, /MYGPTTranslationLoopSendGuard/);
assert.match(source, /autogpt-upload\+visible-attachment/);
assert.match(source, /function attachmentVisible/);
assert.match(source, /function waitForSendReady/);
assert.match(source, /translation-loop-send-ready/);
